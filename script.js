document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- DOM 要素取得（存在しない場合の安全策付き） ---
  const mapSVG = document.getElementById('interactive-map');
  const allPaths = mapSVG ? mapSVG.querySelectorAll('path') : [];
  ...
  // --- 地図パン・ズーム（svg-pan-zoom が無い時は no-op） ---
  const dummyPanZoom = () => ({ zoomIn(){}, zoomOut(){}, resetZoom(){}});
  const panZoomInstance = (window.svgPanZoom ? window.svgPanZoom('#interactive-map', {
    zoomEnabled: true,
    ...
    minZoom: 0.5,
    maxZoom: 10,
    beforePan: (oldPan, newPan) => (isPanning ? newPan : false)
  }) : dummyPanZoom());
  ...
  if (resetZoomBtn) resetZoomBtn.addEventListener('click', () => panZoomInstance.resetZoom());

  // --- データ取得（JSONP、文法エラー修正済み） ---
      ...
      window[callbackName] = function (data) {
        try {
          ...
          if (!data || data.status === 'error') {
            alert('サーバーエラー: ' + (data ? data.message : '不明なエラー'));
            resolve();
            return;
          }
  ...
  function setupDynamicUI() {
    if (!viewSelect || !infoBox || !editPanel) return;
    ...
    if (!viewSelect.value && viewSelect.options.length > 0) {
    ...
    (mapDataHeader || []).forEach(header => {
      ...
      fieldDiv.innerHTML = `<label>${header}:</label>
        <input type="${isCountryField ? 'text' : 'number'}" id="edit-${header}" ${isCountryField ? 'readonly' : ''}>`;
      editPanel.appendChild(fieldDiv);
    });

    if (saveButton) editPanel.appendChild(saveButton);
  ...
  function getColor(stateData) {
    ...
    if (currentView === '国') {
      ...
      return countryInfo ? (countryInfo.Color || '#FFFFFF') : '#FFFFFF';
    } else {
  ...
  function handleLogin() {
    const selected = loginSelect ? loginSelect.value : '';
    loggedInCountry = selected ? selected : null;
    if (loginStatus) loginStatus.textContent = selected ? `${selected} としてログイン中` : 'ログアウト';
  }
  ...
  function handleStateClick(e) {
    ...
    if (editPanel) editPanel.querySelectorAll('input').forEach(input => input.disabled = true);

    const isMyLand = loggedInCountry && stateData && stateData['国'] === loggedInCountry;
    if (isMyLand) {
      if (saveButton) saveButton.style.display = 'block';
      if (editPanel) editPanel.querySelectorAll('input').forEach(input => {
  ...
  function startNationBuilding() {
    ...
    if (nationBuildingControls) nationBuildingControls.style.display = 'block';
    if (startNationBuildingBtn) startNationBuildingBtn.style.display = 'none';
    alert('建国モードを開始します。領土にする空き地を選択してください。');
  }

  function cancelNationBuilding() {
    isNationBuildingMode = false;
    if (nationBuildingControls) nationBuildingControls.style.display = 'none';
    if (startNationBuildingBtn) startNationBuildingBtn.style.display = 'block';
    renderMap();
    nationBuildingSelection = [];
    if (newNationNameInput) newNationNameInput.value = '';
  }

  async function handleConfirmTerritory() {
    const newName = (newNationNameInput ? newNationNameInput.value.trim() : '');
    if (!newName) {
      alert('国名を入力してください。');
      return;
    }
  ...
  // --- 初期化 ---
  if (allPaths) allPaths.forEach(path => path.addEventListener('click', handleStateClick));
  if (loginButton) loginButton.addEventListener('click', handleLogin);
  if (saveButton) saveButton.addEventListener('click', handleSave);
  if (startNationBuildingBtn) startNationBuildingBtn.addEventListener('click', startNationBuilding);
  if (cancelNationBuildingBtn) cancelNationBuildingBtn.addEventListener('click', cancelNationBuilding);
  if (confirmTerritoryBtn) confirmTerritoryBtn.addEventListener('click', handleConfirmTerritory);
  if (viewSelect) viewSelect.addEventListener('change', (e) => { currentView = e.target.value; renderMap(); updateEditPanelVisibility(); });
